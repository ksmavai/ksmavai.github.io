...
export const NoteItem = React.memo(function NoteItem({
  ...
  // Calculate display date based on category
  const displayDate = useMemo(() => {
    return getDisplayDateByCategory(item.category, item.id, item.created_at);
  }, [item.category, item.id, item.created_at]);
  ...
  const NoteContent = (
    <li
      tabIndex={0}
      className={`h-[70px] w-full ${isSelected
  ...
  if (isMobile) {
    return (
      <div {...handlers} className="relative overflow-hidden">
        <div
          data-note-slug={item.slug}
          className={`transition-transform duration-300 ease-out w-full ${isSwipeOpen ? "transform -translate-x-12" : ""
            } ${showDivider
